...
public class SimpleHuffProcessor implements IHuffProcessor {
    ...
    private void treeHeader(BitOutputStream out) {
        ...
        out.writeBits(BITS_PER_INT,
                tree.getNumInternalNodes() + tree.getNumLeafNodes() + tree.getNumLeafNodes() * (BITS_PER_WORD + 1));

        // add the data for the tree
        for (TreeNode node : tree.getAllPreOrder()) {
            if (node.isLeaf()) {
                out.writeBits(1, LEAF_NODE_VAL);
                out.writeBits(BITS_PER_WORD + 1, node.getValue());
            } else {
                out.writeBits(1, INTERNAL_NODE_VAL);
            }
        }
    }
